...
async function sendRequest(requestNumber: number) {
  ...
  try {
    const response = await axios.get(
      `${targetUrl}?t=${new Date().getTime() * Math.random()}`
    );
    ...
    console.log(
      `Request ${requestNumber} - Status: ${response.status} - Time: ${duration}ms`
    );
  } catch (error: any) {
    ...
    console.error(
      `Request ${requestNumber} - Error: ${error.message} - Time: ${duration}ms`
    );
  }
}
...
async function stressTest() {
  ...
  console.log(
    `Successful requests: ${successCount} (${successRate.toFixed(2)}%)`
  );
